...
const confirm = async () => {
    ...
    if (props.type === 'add') {
        if (subcategoryName.value.length === 0) {
            await pushModal(textMsg, { msg: '請輸入子分類名稱' });
            return;
        }
        ...
        const res = {
            ...
            data: {
                ...
                subcategoryShow: subcategoryShow.value
            }
        };
        emit(Modal.EVENT_PROMPT, res);
    }
    else if (props.type === 'edit') {
        const check = props.item.some(item => item.name === name.value && item._id !== id.value);
        if (check === true) {
            await pushModal(textMsg, { msg: `${name.value} 已經存在` });
            return;
        }
...
<template>
    <div class="bg:white p:16|20 r:8 w:full max-w:360">
        ...
        <template v-if="props.type === 'add'">
            <p class="mb:8">子分類名稱</p>
            <input type="text" class="block w:full p:4|8 r:4 b:1|solid|gray mb:16" v-model="subcategoryName">
            <p class="mb:8">子分類中文</p>
            <input type="text" class="block w:full p:4|8 r:4 b:1|solid|gray mb:16" v-model="subcategoryShow">
        </template>
        <div class="h:16"></div>
